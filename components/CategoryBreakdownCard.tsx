import type { RootState } from "@/redux/store";
import { globalStyles } from "@/styles/globalStyles";
import { formatCurrency } from "@/utils/currencyUtils";
import React from "react";
import { Text, View } from "react-native";
import { useSelector } from "react-redux";

const CategoryBreakdownCard: React.FC = () => {
	const expenses = useSelector((state: RootState) => state.expenses || []);
	const categories = useSelector((state: RootState) => state.categories || []);
	const currency = useSelector((state: RootState) => state.settings?.currency || "CAD");
	const exchangeRates = useSelector((state: RootState) => state.settings?.exchangeRates);

	const totalExpenses = expenses.reduce((sum, exp) => sum + (exp?.amount || 0), 0);

	const breakdown: Record<string, number> = {};
	expenses.forEach((exp) => {
		if (exp?.category?.id) {
			breakdown[exp.category.id] = (breakdown[exp.category.id] || 0) + (exp.amount || 0);
		}
	});

	return (
		<View style={globalStyles.cardWithShadow}>
			<Text style={globalStyles.cardTitle}>Category Breakdown</Text>
			{Object.keys(breakdown).length > 0 ? (
				Object.entries(breakdown)
					.sort((a, b) => b[1] - a[1])
					.slice(0, 5)
					.map(([key, value]) => {
						const category = categories.find((c) => c?.id === key);
						const percentage = totalExpenses > 0 ? ((value / totalExpenses) * 100).toFixed(0) : "0";
						return (
							<View
								key={key}
								style={globalStyles.legendItem}
							>
								<View style={[globalStyles.legendColor, { backgroundColor: category?.color || "#999" }]} />
								<View style={globalStyles.legendInfo}>
									<Text style={globalStyles.legendText}>{category?.name || String(key)}</Text>
									<Text style={globalStyles.legendPercentage}>{String(percentage)}%</Text>
								</View>
								<Text style={globalStyles.legendAmount}>{formatCurrency(value || 0, currency, exchangeRates?.rates)}</Text>
							</View>
						);
					})
			) : (
				<Text style={globalStyles.placeholderText}>No expenses recorded yet</Text>
			)}
		</View>
	);
};

export default CategoryBreakdownCard;
