...
const BudgetCard: React.FC = () => {
	const totalExpenses = useSelector((state: RootState) => state.expenses?.reduce((sum, exp) => sum + (exp?.amount || 0), 0) || 0);
	const budget = useSelector((state: RootState) => state.settings?.budget || 1000);
		...
		<View style={globalStyles.cardWithShadow}>
			...
			<View style={globalStyles.progressContainer}>
				<View style={globalStyles.progressBarBackground}>
					<View style={[globalStyles.progressBarFill, { width: `${Math.min(budgetProgress, 100)}%`, backgroundColor: getBudgetStatusColor() }]} />
				</View>
				<View style={globalStyles.budgetInfo}>
					...
					<Text style={[globalStyles.budgetPercentage, { color: getBudgetStatusColor() }]}>{budgetProgress.toFixed(0)}%</Text>
				</View>
				<Text style={[globalStyles.budgetStatus, { color: getBudgetStatusColor() }]}>{getBudgetStatusText()}</Text>
				<Text style={globalStyles.budgetRemaining}>{formatCurrency(Math.max(budgetRemaining, 0), currency, exchangeRates?.rates)} remaining</Text>
			</View>
		</View>
	);
};
