...
export default function SettingsScreen() {
	...
	const handleToggleQuote = () => {
		...
		showAlert("Inspiration Quote", newValue ? "Inspiration quotes enabled" : "Inspiration quotes disabled");
	};

	const handleToggleAdvice = () => {
		dispatch(toggleShowAdvice());
		const newValue = !showAdvice;
		showAlert("Daily Advice", newValue ? "Daily advice enabled" : "Daily advice disabled");
	};
	...
	return (
		<ScrollView contentContainerStyle={styles.container}>
			...
			<SettingsSection title="Display Currency">
				...
				/>
				<Text style={styles.currencyNote}>Note: All expenses are stored in CAD. This only affects display.</Text>
				{exchangeRates?.lastUpdated && <Text style={styles.rateUpdateText}>Rates updated: {new Date(exchangeRates.lastUpdated).toLocaleDateString()}</Text>}
			</SettingsSection>
